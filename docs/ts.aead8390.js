...
(function (domain) {
  ...
  function () {
    ...
    Time.prototype.formatedValue = function () {
      return Time.zerofil2(this.hours) + ":" + Time.zerofil2(this.minutes);
    };

    Time.createFromTotalMinutes = function (m) {
    ...
    Time.parse = function (text) {
      if (text.indexOf(':') != -1) {
        var p = text.split(':').map(function (v) {
          return parseInt(v);
        });
  ...
  function () {
    ...
    TodoTask.doing = function (title, estimateTime, startTime) {
      return new TodoTask(title, estimateTime, startTime, null);
    };

    TodoTask.done = function (title, estimateTime, startTime, endTime) {
...
},{"marked":"3F7m"}],"LQOA":[function(require,module,exports) {
"use strict";
...
var markdowntodo = __importStar(require("./markdowntodo"));

var domain = markdowntodo.domain;
var view;
...
(function (view) {
  ...
  function refresh() {
    var markdownTodo = markdowntodo.createMarkdownTodo(editorElement.value.trim());
    resultElement.innerHTML = '<table><tr><th>タイトル</th><th>見積もり</th><th>区分</th><th>開始</th><th>終了</th></tr><tr>' + markdownTodo.todoOrSchedules.filter(function (v) {
      return !v.isDone;
    }).map(function (v) {
      var text = [v.title, (v.estimateTime ? v.estimateTime.totalHours : '') + 'h', v.isTodo ? 'TODO' : '予定', v.startTime ? v.startTime.formated : '', v.endTime ? v.endTime.formated : ''].filter(function (v) {
        return v !== null;
      }).join('</td><td>');
      return "<td>" + text + "</td>";
    }).join('</tr><tr>') + '</tr></table>' + ("<h3>\u6B8B\u308A\u5408\u8A08: " + domain.Time.createFromTotalMinutes(markdownTodo.todoOrSchedules.filter(function (v) {
      return !v.isDone;
  ...
  document.querySelector('#editor').addEventListener('keydown', typeTab('  ')
  /* スペース2つ */
  );
